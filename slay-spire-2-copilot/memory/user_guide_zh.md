...
- 每局笔记（`memory/runs/` 下的英文 + `_zh` 中文对）：格式遵循
  `memory/template.md`
- 跨局总览（`memory/overview.md`）：两个二级标题节——`## 对局表`
  （对局大表；规则见其表格规则块）与 `## 统计`（交叉统计表，由对局表
  重新核算得出）

## 写入时机

- **run 笔记 + overview 行 + 统计**：每局结束立即写——`memory/runs/` 下
  的笔记对、overview.md 对局表顶部插入一行，以及 `## 统计` 交叉表
  （战绩总览 / 进阶进度）按对局表重新核算
- **阶段性反思**：第一次覆盖当时为止全部对局；此后**每 10 局**一次
  （如 36–45、46–55、56–65……），累计对局数到达边界时触发。反思内容**直接写入
  `memory/lessons/` 对应的分类 markdown**，绝不写入 overview.md，且
  **每一条都带日期**。overview.md 只放整体对局结果（表格），不放任何
  经验性内容
- **lessons/**：本局前向经验与阶段性反思结论写入对应分类文件——能与
  条目清单表格中的某一行一对一对应的条目，写入该行「经验」格（形式仍为
  `- (日期) …` 无序列表项）；无唯一对应行的，以无序列表附在表格之后。
  反思沉淀的条目逐条标注日期
...
- 原则：任何决策动念时先查历史经验再选，绝不凭印象行事；**每步决策之前
  只查最小必要内容**——只打开与该决策直接相关的那一条 lessons 条目或
  lookup 结果，不做全量翻阅
