...
- STONE_OF_ALL_TIME（永恒之石；Act 1；所有玩家 ≥1 瓶药水）：Lift → +10 最大 HP（饮用药水路径——精确消耗 1 瓶药水）；Push → 6 点伤害 + 附魔（+8 Vigorous 变量）。run-33 A1 实测（2026-09-18）：Lift 饮下 1 瓶敏捷药后同次结算 **+10 最大 HP 且 +10 当前 HP**（72/80 → 最大值升至 90 后 82/90）——当前 HP 补足属于效果本体而非单独治疗；按 1 瓶药换 +10/+10 记账。
...
- TABLET_OF_TRUTH（真理石板）：Smash → 回复 20（run-33 实测再确认 2026-09-18：69/80 → 80/80——20 点治疗溢出封顶至最大值；最大 HP 陷阱定性不变，Smash 为正确线）。Decipher → 递增的最大 HP 阶梯（run-20 A1 实测 2026-09-18）：各级代价 −3/−6/−12 Max HP（翻倍；loc 键 DECIPHER_1/2/3，每级选项"继续解读"），每级均可"放弃"；**任何已完成级别与放弃均未观察到奖励**——实测为纯最大 HP 消耗事件；原一行摘要"失去 3 最大 HP + 升级路径"判定为错误，除非另有来源确认奖励路径，否则应选 Smash。run-20 三级+放弃共付 21 Max HP（85→64）换零收益。

run-34 事件折档（A1，2026-09-18，种子 BZS6HFJL7Y）：**VAKUU（Act3 荣光远古；瓦库）选项解码（relics.md 引用+run-34 live 一选）**：血染玫瑰 → BloodSoakedRose 遗物：每回合 +1 最大能量，但拾取时向牌库加入 1 张 **Enthralled**——Enthralled = 2 费诅咒、永恒、**在手时必须先于其他牌打出**（每次出现税 2 能、付清前封锁其他出牌——run-34 拒绝；+1能/回合无法对冲爆发线破坏）；原初之爪 → 拾取加 2 随机诅咒+3 张 Wish（诅咒代价类，教条拒绝）；选择悖论 → 第 1 回合开始从 5 张随机卡选 1 加入手牌；这 5 张均获 Retain（run-34 live 一选——**语义 live 解码：添加为带 Retain 的本战斗手牌添加，每战 T1 新五选一，非永久入牌库**；live 三连战选上勾拳/祭品/怨恨均进入击杀线）。注：档案反编译此前缺 Vakuu 选项处理器——本折经 relics.md+live 补齐该缺口。**脑蛭 run-34 live 订正**：RIP −5 精确；**live 卡牌选择给 3 选项，非档案「5 选 1」**——取 Omnislice（0费8+溅射）；5 选 1 说法订正为 3 选 1 级待更多 live。**满屋芝士 run-34 再确认**：SEARCH −14 精确换天选芝士；Gorge 类加牌选项仍按教条拒绝（HP 经济遗物优先于卡牌稀释）。**TEZCATARA run-34 live 一选**：营养汤 → 所有 Strike 标签基础牌附魔余烬（0费+3伤永恒）——**run-34 定调一选：4 张基础 Strike 全程变为免费 12-19 伤卡**；同屏烘焙手套拒绝（档案 run-33 Headbutt 反协同）；南瓜蜡烛同屏（relics.md 解码：+1能/回合、5 战后熄灭、休息点 KINDLE 选项）——因短暂性+休息点行动税拒绝。
...
- OROBAS（欧洛巴斯；Act2 虫巢远古；2026-09-17 run-19 perplexity 研究+局内实测）：ELECTRIC_SHRYMP 放电异虾 → 拾取时选 1 张技能牌附魔 **Imbued**（Imbued = 战斗开始自动打出、不耗能量——每战等效免费）；DRIFTWOOD 浮木 → 每张卡牌奖励可重掷 1 次；TOUCH_OF_OROBAS 欧洛巴斯之触 → 将初始遗物替换为远古版（IRONCLAD：燃烧之血每战+6 → **黑暗之血每战回 12**；run-19 实测：遗物 id 更换，战斗结束回 12 与天选芝士 +1 Max HP 同战叠加）。 **run-27 全池解码（perplexity 2026-09-18）+局内实测**：本局提供 SAND_CASTLE/RADIANT_PEARL/TOUCH_OF_OROBAS。全选项池：ELECTRIC_SHRYMP（1 技能牌 Imbued 附魔）、GLASS_EYE（2 普通+2 罕见+1 稀有卡）、SAND_CASTLE 沙堡（**拾取时随机升级 6 张牌**）、ALCHEMICAL_COFFER（**4 药槽填满随机药水**）、DRIFTWOOD（卡牌奖励重掷）、RADIANT_PEARL 发光珍珠（**每战开局手牌加 1 张 Luminesce**）、PRISMATIC_GEM（回合开始 +1；卡牌奖励混入他色牌）、SEA_GLASS（查看 15 张他角色牌选任意张）、TOUCH_OF_OROBAS（起始遗物→远古版）、ARCHAIC_TOOTH（1 张起始牌变化为远古版）。run-27 选欧洛巴斯之触→黑暗之血 12/战实弹（该 Act 87→94/83→94/48→60/6→18/17→29/38→50——Act2 HP 经济生命线）。
