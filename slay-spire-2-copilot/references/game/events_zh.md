...
- NEOW（尼奥）：远古系开局赐福，第一章（蔓生之地 Overgrowth）唯一的 Ancient（2026-09-17 反编译，游戏 v0.107.1）。选项来自两个遗物池：正面遗物（Arcane Scroll、Booming Conch、Fishing Rod、Golden Pearl、Kaleidoscope、Lead Paperweight、Lost Coffer、Massive Scroll、Neow's Torment、New Leaf、Phial Holster、Precise Scissors、Scroll Boxes、Winged Boots，另有 Lava Rock / Neow's Talisman / Nutritious Oyster / Pomander / Small Capsule / Stone Humidifier）与诅咒代价遗物（Cursed Pearl、Hefty Tablet、Large Capsule 等，完成文案为 CURSED）。优先选正面；诅咒代价无充分理由不选。
- 章节起始远古房（实战漏走 2026-09-17，用户确认；同日反编译）：每章地图最低一排是 Ancient/尼奥系赐福房——Act 2 起点 (0,3)=Ancient，先古移民选项含恢复血量（用户确认是非常强的 buff）。**第一章门槛（反编译）**：`ActModel.GenerateRooms` 从 `GetUnlockedAncients` 抽取本章 Ancient；Overgrowth 会在 `UnlockState.IsEpochRevealed<NeowEpoch>()` 为假时移除 Neow，且 `RunManager.GenerateMap` 在 `ExtraFields.StartedWithNeow` 为假时把第一章 StartingMapPoint 改型为 `Monster`——未揭示 NeowEpoch 的存档根本没有第一章赐福房。手动游玩在 Timeline 界面揭示（打开界面=自动 ObtainEpoch，点槽位=RevealEpoch）；自动化对局从不进 Timeline。修复：bridge mod 的 `start_run` 现在在 embark 前调用 `EnsureNeowEpochRevealed()`（ObtainEpochOverride → EpochState.Revealed + 存档）；标志置位后 `RunManager.EnterAct` 会在开局自动进入尼奥事件房（state 为 event 界面而非地图）。后续章节：Glory/Hive 的远古房不受门槛限制（Hive 仅 Orobas 受 OrobasEpoch 限制）；章节切换后仍落在第 1 排地图——先处理 `run.act_start_room`（visited=false 且 is_boon_room=true 或 point_type 为 Ancient）——最先 map_select 该点；state 会把它重新注入 available_map_points 头部，紧凑视图会打印 ACT-START BOON ROOM UNVISITED 警告。跳过赐福是永久损失（地图不能回头）。
