...
- Cure All（痊愈药水）(Uncommon, 战斗中, 任意玩家): 获得 1 能量；抽 2 张牌。名称像治疗但**不是治疗**——爆发/挖牌类（+1能+抽2）；任何「CureAll 治疗」注记均为错误。
...
- Duplicator（复制药水）(Uncommon, 战斗中, 自身): 获得 DuplicationPower 1——本回合你打出的下一张牌额外打出 1 次；回合结束移除。使用后 DUPLICATION_POWER:1 同帧可见，下一张攻击双结算（如 Bash+ 双结算）；药水消费后 DuplicationPower 随回合结束消失。
...
- Fairy in a Bottle（瓶中精灵）(Rare, 自动, 自身): 将死时改为弃掉此药水并回复至最大 HP 的 30%（最少 1 点）。在 HP 1 被回合开始自伤击杀时触发——HP 置为 Max 的 ~29.6%，与 30% Max 规则吻合；药水同帧从药槽消失。
...
- Foul Potion（污浊药水）(Event, 随时): 战斗中——对每个非宠物生物（含你自己）造成 12 点伤害；商人房间——获得 100 金币；FakeMerchant 事件——触发伪商人战斗。战斗用法=同窗 AoE 工具——12 伤可直接带走 12 HP 千足虫段并铺垫 Thunderclap 收头；自伤触发 Rupture（按 Amount +力）。伪商人房绝不可用（战斗条款）——留给真商人房 +100 金是经济线。
- Fruit Juice（果汁）(Rare, 随时, 任意玩家): +5 最大 HP。
- Fysh Oil（异鱼之油）(Uncommon, 战斗中, 任意玩家): 本场战斗 +1 力量 +1 敏捷。使用后同帧 STRENGTH_POWER +1 与 DEXTERITY_POWER:1；敏捷 +1 使后续卡牌挡值各 +1（Defend 5→6 级）同战生效。使用时机存在异文：另有解码记为「随时/自身」、同样立刻 +1/+1——按 build 核实时机。
...
- Liquid Memories（液态记忆）(Rare, 战斗中, 自身): 从弃牌堆选 1 张牌加入手牌，本回合免费。弃牌堆为空时不可用——空弃牌堆打出时药水被消耗但**无选牌界面、无任何效果**（完全空耗）。使用前先确认 piles。
...
- Soldier's Stew（士兵炖汤）(Rare, 战斗中, 任意玩家): 本场战斗内你战斗牌堆中所有含 Strike 标签的卡牌 +1 次 Replay。Replay 表现为用药时对各张卡的一次性充能 —— 用药时已在手牌/抽牌堆中的打击牌打出时额外结算一次；战斗后期才抽到的打击只结算一次。
...
- Fruit Juice：+5 最大 HP 已确认（多源研究；STS2 与 STS1 行为一致）。亦可作 StoneOfAllTime Lift 燃料（事件的 +10 Max HP 路径——优于单独喝掉）。可见于商店在售与战斗奖励。
- 速度药水 Speed Potion：**不是抽牌药水**（与 Swift Potion 区分）；敏捷增益次回合过期（战斗增益类，回合数封顶——按同回合/短窗价值处理）。速度药水+敏捷药水叠用：DexPotion +2（战斗持续）再 SpeedPotion +5（当回合，SPEED_POTION_POWER:5 计数）→ 同读 DEXTERITY_POWER:7；Speed 部分次回合过期（敏捷回到持久 +2）。
- 缚魂药水 Potion of Binding：decomp OnUse 中变量名互换（WeakPower 用了 Vulnerable 的值、反之亦然），两者数值均为 1——表值为全体 1 Weak + 1 Vulnerable。单体样本曾读到施加 Weak 3 + Vuln 1（意图 19→14 弱化）——层数按使用时 live 读数核实；全体自动规则成立（无需 target id）。
- Blood Potion：20% Max 公式精确（80 Max 时回复 +16 HP）。
- 卡牌选择类药水（Attack/Skill/Power/Colorless 类）：生成牌本回合免费，即使印刷费用 2–3；选择界面可跳过。无色池包含高费卡如 Eternal Armor（PLATING_POWER 9——回合结束 9 挡，之后每回合 −1；结构性反属性吸取挡类）。
- 铁心 HeartOfIron（药水）：对玩家施加 **PLATING_POWER 7**——时机见 powers.md 玩家镀层条目（战斗中途使用在玩家侧回合结束时给挡）。
- 药槽满 3/3 时卡牌奖励按钮仍可领取（未观察到死按钮锁死类）。
