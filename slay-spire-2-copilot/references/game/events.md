...
- SELF_HELP_BOOK: live option keys (run-20 A1 2026-09-18) are READ_THE_BACK / READ_PASSAGE — no SkipBook button in the in-game UI; ReadTheBack is undocumented in decomp extracts (do not blind-click; research or take ReadPassage). READ_PASSAGE → deck_select enchant screen; run-20 observed the pool as skills-only (4× Defend + Second Wind offered — attacks/Bash not eligible). Enchant live value on Second Wind: +2 Block per exhausted card (5→7); a later smith upgrade on that enchanted copy added +2 more (7→9 per card — NOT the table's +7). Prior summary: ReadPassage/ReadEntireBook enchant Sharp/Nimble/etc.; SkipBook leaves.
