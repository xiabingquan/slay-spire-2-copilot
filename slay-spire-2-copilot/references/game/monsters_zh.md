...
- TheInsatiable 无底之欲（TheInsatiable, Act2 TheInsatiableBoss; 321 HP）：LIQUIFY_GROUND_MOVE = 增益；THRASH_MOVE / THRASH_MOVE_2 = 多段攻击 8 (A:9) ×2；LUNGING_BITE_MOVE = 单次攻击 28 (A:31)；SALIVATE_MOVE = 增益 自身 +2 力量 (A:3)。循环：LIQUIFY → THRASH → SALIVATE → THRASH_2 → LUNGING_BITE → THRASH… run-20 A1 实测（2026-09-18，state 名 无厌沙虫）：SANDPIT_POWER 自 T2 起向抽牌堆注入 FranticEscape 陷阱；实测循环 LIQUIFY(Buff) → THRASH 8×2 → LUNGING_BITE 28 → SALIVATE(Buff) → 力量堆叠 THRASH 10×2；T5 显示意图 10×2 在 9 格挡下结算致死（预期穿挡仅约 11）且蜥蜴尾巴未触发——隐藏伤害源或多段力量算式未解；与 run-16 同 Boss 同死法。**run-22 A1 实测（2026-09-18，Seed LJ4JNPVHVM，state 名 无厌沙虫）——同款第三例**：55/84 进场，T1-T5 将 Boss 磨到 166（FranticEscape 至少 4 次上手，一次未打）；T5 结束时玩家 HP 45、挡 15（Crimson Mantle 8 + Taunt 7）对显示意图 `7×2=14`——数学上零损回合——敌方回合 HP 结算为 0，Boss 剩 160/321。**该 Boss 的 THRASH 类多段显示意图不可用于挡值算术；无论显示意图 vs 挡值总量如何，每一回合都按潜在致死处理**。
