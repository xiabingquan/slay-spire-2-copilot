...
3. `... act <动作> --args '<json>' --wait` — action 立即返回（已提交）；`--wait`
   轮询至状态稳定（指纹不再跳动）后打印。**每次只打一张牌 — 硬规则（用户指令
   2026-09-18）**：战斗中的卡牌打出**绝不批量提交**。每次 `act play` 只打**一**
   张牌，然后重读 state，再根据最新下标决定下一张——禁止多卡 `batch` 倾泻、
   禁止预计算出牌链。从根源上杜绝索引漂移（该过程债几乎每局 A1 都复发——
   run-4/14–25 每局约 10–12 次，run-25 皇蟹 RECHARGE 爆发窗一次漂移损失约
   50 伤）。`end_turn` 是独立一次调用，且仅在重读 state 确认无后续出牌后才
