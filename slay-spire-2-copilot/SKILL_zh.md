...
5. **阶段性反思**：`memory/overview.md` 顶部设有 `# 阶段性反思`
   一级标题节，每次反思在该节下新增一条 `## 反思 (yyyy-mm-dd)` 二级标题。
   节奏：**第一次反思覆盖目前为止的全部对局**；从第二次起，每次覆盖自上次
   反思以来的 **20 局**（即第二次覆盖对局 36–55，第三次覆盖 56–75，依此类推，
   累计对局数到达该边界时触发）。素材是 `memory/runs/` 下覆盖区间内各局的
   记录——先读对应对局笔记，再以结构化形式（markdown 表格、有序/无序列表）
   反思得失。反思是 `overview.md` 中唯一可以写教训的位置；对局表格本身仍然
   只写 meta。表格最前面另加一列 **对局序号**，按时间顺序从 1 开始编号
   （最新对局置顶，序号最大）。
